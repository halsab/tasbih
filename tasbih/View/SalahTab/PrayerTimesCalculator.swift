...
final class PrayerTimesCalculator {
    ...
    private lazy var jDate:Double = PrayerTimesCalculator.julianDate(from: Date())
    ...
    func getDatePrayerTimes(year:Int, month:Int, day:Int, latitude:Double, longitude:Double, tZone:Float)->[TimeNames: Any] {
        ...
        comp.month = month
        comp.day = day
        calcDate = PrayerTimesCalculator.gregorianCalendar.date(from: comp)
        
        timeZone = tZone
        
        jDate = PrayerTimesCalculator.julianDate(year: year, month: month, day: day)
    ...
    func floatToNSDate(_ time:Double)->Date? {
        if let (hours, minutes) = floatToHourMinute(time) {
            var components = PrayerTimesCalculator.gregorianCalendar.dateComponents(PrayerTimesCalculator.calendarComponentsDMY, from: calcDate)
            components.hour = hours
            components.minute = minutes
            return PrayerTimesCalculator.gregorianCalendar.date(from: components)
        } else {
            return nil
        }
    }
    ...
    private func calculateJulianDate() {
        if let date = calcDate {
            if let latlng = coordinate {
                jDate = PrayerTimesCalculator.julianDate(from: date)
                jDate = jDate - (latlng.longitude / (15.0 * 24.0))
            }
        }
    }
    ...
    private class func julianDate(from date:Date)->Double {
        let components = PrayerTimesCalculator.gregorianCalendar.dateComponents(PrayerTimesCalculator.calendarComponentsDMY, from: Date())
        return julianDate(year: components.year ?? 0,
                          month: components.month ?? 0,
                          day: components.day ?? 0)
    }
    ...
    private func computeDayTimes()->[TimeNames: Any] {
        //default times
        let times = PrayerTimesCalculator.DefaultDayTimes;
